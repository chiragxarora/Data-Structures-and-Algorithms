...
int LCS(string s1, string s2) {
    ...
    if(s1[s1.length()-1]==s2[s2.length()-1]){
        ...
        return LCS(s1.substr(0,s1.length()-1),s2.substr(0,s2.length()-1))+1;
    } else {
        // otherwise check by removing next character of each string one by one
        int option1 = LCS(s1.substr(0,s1.length()-1),s2);
        int option2 = LCS(s1,s2.substr(0,s2.length()-1));
...
int LCS_BUSE(string s1, string s2) {
